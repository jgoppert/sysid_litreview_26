...
./results.py all-6dof
\end{verbatim}
which generates the selected 6-DOF train/validation data, runs the implemented 6-DOF baseline methods, exports website data, and refreshes LaTeX-ready artifacts. The 6-DOF dataset contains inertial position, body velocity, quaternion attitude, body angular rates, pilot commands, actuator-realized commands, noisy direct-state measurements, mocap-style position/quaternion measurements, true aerodynamic coefficients, attached-flow nominal coefficients, and coefficient residual labels.
...
\begin{figure}[hbt!]
    ...
    \caption{Direct-state 6-DOF benchmark heatmap sorted by validation score. The score is full-state mean NRMSE over the selected 6-DOF validation rollouts; the default plotted dataset is the aggressive nonlinear stall case.}
