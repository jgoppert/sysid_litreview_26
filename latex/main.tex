...
The 6-DOF nominal row propagates the repository's quaternion rigid-body smoke model with the supplied pilot-command history. It has no fitted parameters and is used as the no-fit baseline for the developing coupled-dynamics benchmark.
...
\]
This tests the same grey-box idea used in UDE-style methods, but with a linear residual so that the initial 6-DOF benchmark remains fast and deterministic.

\paragraph{6DOF-MocapOutputARX.}
...
The current dynamics are intentionally compact so the benchmark interface can stabilize before adding a high-fidelity 6-DOF aerodynamic model. Body translational dynamics use a damped velocity model with gravity resolved into the body frame and command-dependent axial, side, and normal accelerations. Rotational dynamics use diagonal inertia, command moments, and rate damping. The quaternion is renormalized after every RK4 step. This model is not intended to replace the 3-DOF nonlinear aerodynamic benchmark; it provides a complete data and method path for future 6-DOF methods.
...
\begin{figure}[hbt!]
    ...
    \caption{6-DOF baseline method validation scores. Direct-state rows score full-state mean NRMSE. The mocap-output ARX row scores position/quaternion output NRMSE because it does not predict body velocities or angular rates. Lower is better.}
...
\begin{figure}[hbt!]
    ...
    \caption{Representative 6-DOF validation rollout for the best implemented state-predictor baselines. Rollouts use only the validation initial condition and pilot-command sequence after training.}
