...
Model stitching is a related surrogate strategy that trades a single global model for a collection of local models. The idea appears in local-expert and mixture-of-experts models \citep{jacobs1991adaptive}, multiple-model nonlinear modeling and control \citep{murray1997multiple}, and LPV identification \citep{toth2010lpv}. In aircraft applications, the scheduling variables may include airspeed, angle of attack, pitch rate, throttle, elevator, flight mode, or a hidden-controller indicator. A local linear or local aerodynamic-coefficient model can be accurate near trim, another near high-angle-of-attack stall onset, and another during recovery. The stitch may be a hard switch, a gain-scheduled interpolation, or a probabilistic mixture. This is attractive for small-UAS benchmarks because aggressive maneuvers deliberately move across regimes where one global linearization is inadequate. The drawbacks are equally important: regime boundaries must be excited by data, interpolation can create nonphysical force or moment behavior, and hard switching can introduce discontinuities unless the stitched model is constrained carefully.
...
excursions. This design separates the question ``does a local linear model work
near trim?'' from the harder question ``does one global model remain adequate
across trim, stall onset, recovery, and speed variation?''
...
The stitched local-model row first clusters training trajectories by their
initial state $\mathbf{x}_{i,0}$. With $M$ local models, k-means assigns each
training trajectory to a regime index $r_i\in\{1,\ldots,M\}$. For each regime,
the code fits the same affine one-step model as \eqref{eq:code_linear_ss},
\begin{equation}
    \widehat{C}^{(m)}
    =
    \arg\min_C
    ...
    \mathbf{x}_{i,k+1}
    -
    [1,\mathbf{x}_{i,k}^T,\mathbf{u}_{i,k}^T]C
    \right\|_2^2
    +\lambda\|C\|_F^2.
    \label{eq:code_model_stitching_local}
\end{equation}
Each local model is associated with the mean state
$\boldsymbol{\mu}^{(m)}$ of its assigned samples. During rollout, the local
predictions
\[
    \hat{\mathbf{x}}_{k+1}^{(m)}
    =
    [1,\hat{\mathbf{x}}_k^T,\mathbf{u}_k^T]\widehat{C}^{(m)}
\]
...
\begin{align}
    ...
    \left\|S_x^{-1}(\hat{\mathbf{x}}_k-\boldsymbol{\mu}^{(m)})\right\|_2^2\right]
    }{
    \sum_j
    \exp\left[-\frac{1}{2\ell^2}
    \left\|S_x^{-1}(\hat{\mathbf{x}}_k-\boldsymbol{\mu}^{(j)})\right\|_2^2\right]
    },\\
    \hat{\mathbf{x}}_{k+1}
...
centers in normalized state space. Validation is an open-loop rollout from the
validation initial condition using only the pilot-command history. This is
...
\begin{figure}[H]
    ...
    \caption{Training-time versus validation-error tradeoff for the near-trim open-loop benchmark. The vertical axis is $J_{\mathrm{val}}$, the mean of the range-normalized RMSEs for $V,\alpha,\gamma$, and $q$ over all validation samples. Marker area increases with validation rollout time. The red dashed horizontal line gives the nominal-model baseline, which has no fitted training solve. Lower vertical position means lower trajectory error and therefore better validation performance.}
