...
\begin{enumerate}
    ...
    \item a developing 6-DOF aircraft dataset interface used to stabilize future coupled-dynamics benchmarks;
...
\end{verbatim}
and currently writes \texttt{methods/data/aircraft\_6dof\_mixed/}. This dataset stabilizes the state, input, position, quaternion-attitude, and mocap-style measurement interface for future 6-DOF benchmark work. It uses the current smoke dynamics and should not yet be interpreted as the final nonlinear 6-DOF aerodynamic benchmark.
...
The immediate roadmap is repository-driven rather than submission-driven. The first priority is to keep the 3-DOF benchmark reproducible: generated datasets, figures, tables, website artifacts, and this reference document should be regenerated from the same command-line entry points. The second priority is to mature the developing 6-DOF dataset interface into a full nonlinear benchmark with coupled lateral-directional and longitudinal dynamics. The third priority is to stabilize the method-plugin interface so external contributors can add methods without editing the core comparison harness.
