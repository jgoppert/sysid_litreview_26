...
The 6-DOF dataset interface now mirrors the 3-DOF split between local, frequency-excitation, aggressive, and trim-grid cases. The default command
\begin{verbatim}
...
./results.py all-6dof
\end{verbatim}
which generates the standard 6-DOF train/validation datasets, runs the implemented 6-DOF baseline methods on each dataset, exports website data, and refreshes LaTeX-ready artifacts. The 6-DOF dataset contains inertial position, body velocity, quaternion attitude, body angular rates, pilot commands, actuator-realized commands, noisy direct-state measurements, mocap-style position/quaternion measurements, true aerodynamic coefficients, attached-flow nominal coefficients, and coefficient residual labels.
...
\end{align}
The nominal 6-DOF model used by \texttt{nominal\_rk4\_step} sets $\sigma=0$ and removes every explicitly nonlinear stall/residual term in \eqref{eq:sixdof_cl_truth}--\eqref{eq:sixdof_cn_truth}. The truth model used by \texttt{rk4\_step} keeps those terms. This nominal/truth split is the 6-DOF analogue of the hidden residual-aerodynamics problem in the 3-DOF benchmark.
...
\begin{figure}[hbt!]
    ...
    \caption{Direct-state 6-DOF benchmark heatmap sorted by mean validation score. The score is full-state mean NRMSE. Columns report the mean across 6-DOF datasets followed by the open-loop, sine-sweep, aggressive nonlinear stall, and trim-grid validation datasets.}
...
\begin{figure}[hbt!]
    ...
    \caption{Mocap-derived 6-DOF benchmark heatmap sorted by mean validation score. These rows expose the same pilot commands as the direct-state case but initialize and train from position/attitude-derived state estimates. Columns report the mean across 6-DOF datasets followed by the open-loop, sine-sweep, aggressive nonlinear stall, and trim-grid validation datasets.}
