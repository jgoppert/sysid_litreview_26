...
def write_method_table(rows: list[dict[str, str]], output: Path, caption: str, label: str) -> None:
    ...
    column_spec = "llrrrrrrrl" if show_source else "lrrrrrrrl"
    header = (
        r"Method & Source & Score & Train [s] & CPU [s] & GPU [s] & GPU MB & Rollout [s] & $V$ RMSE & Backend \\"
        if show_source
        else r"Method & Score & Train [s] & CPU [s] & GPU [s] & GPU MB & Rollout [s] & $V$ RMSE & Backend \\"
    )
        ...
        for row in rows:
            ...
            values.extend(
                [
                    ...
                    ffloat(row.get("train_cpu_s")),
                    ffloat(row.get("train_gpu_s")),
                    ffloat(row.get("gpu_memory_mb")),
